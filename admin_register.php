			...
			if(($firstname != '') || ($middlename != '') || ($lastname != '') || ($password != '') || ($username != ''))
			{
				...
				if($sthandler->rowCount() <= 0)
				{
					...
					if($sthandler1->rowCount() <= 0){
						add_admin( $username, $password, $firstname, $middlename, $lastname, $position);
					$full = "Welcome Admin ".$firstname. " " .$middlename. " ".$lastname. " ". "You are successfully registered. Enjoy your work!";
					$msg = 
							My_Alert($full);  
					}
					else{
						$full = "Your username already existed! Please try another :D";
					$msg = 
							My_Alert($full);
					}
				}
...
<h1>Administrator Registration</h1>
     ...
     <form method="post">
         ...
         <p><a href="admin_checkin.php">Login?</a></p>					 
     </form>
	 </div>
	 </div> 
</div>
